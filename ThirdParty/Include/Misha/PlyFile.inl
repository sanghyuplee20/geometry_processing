...
PlyFile *PlyFile::_Write( FILE *fp , const std::vector< std::string > &elem_names , int file_type )
{
	...
	for( unsigned int i=0 ; i<elem_names.size() ; i++ )
	{
...
#if _WIN32 || _WIN64
	FILE *fp;
	if( fopen_s( &fp , name.c_str() , "wb" ) ) MK_ERROR_OUT( "Failed to open file for writing: " , name );
#else // !_WIN32 && !_WIN64
...
void PlyFile::describe_element( const std::string &elem_name , size_t nelems , int nprops , const PlyProperty *prop_list )
{
	...
	if( elem==NULL ) MK_ERROR_OUT( "Can't find element '" , elem_name , "'" );
...
void PlyFile::describe_other_properties( const PlyOtherProp &other , int offset )
{
	...
	for( unsigned int i=0 ; i<other.props.size() ; i++ ) elem->props.push_back( PlyStoredProperty( other.props[i] , OTHER_PROP ) );
...
void PlyFile::header_complete( void )
{
	...
	switch( file_type )
	{
		...
		default: MK_ERROR_OUT( "Bad file type: " , file_type );
	}

	/* write out the comments */
	for( unsigned int i=0 ; i<comments.size() ; i++ ) fprintf( fp , "comment %s\n" , comments[i].c_str() );

	/* write out object information */
	for( unsigned int i=0 ; i<obj_info.size() ; i++ ) fprintf( fp , "obj_info %s\n" , obj_info[i].c_str() );
	...
	for( unsigned int i=0 ; i<elems.size() ; i++ )
	{
		...
		for( unsigned int j=0 ; j<elems[i].props.size() ; j++ )
		{
...
void PlyFile::put_element( void *elem_ptr )
{
	...
	if( file_type==PLY_ASCII )	/* write an ascii file */
	{
		...
		for( unsigned int j=0 ; j<elem->props.size() ; j++ )
		{
...
PlyFile *PlyFile::_Read( FILE *fp , std::vector< std::string > &elem_names )
{
	...
	for( unsigned int i=0 ; i<plyfile->elems.size() ; i++ )
	{
		for( unsigned int j=0 ; j<plyfile->elems[i].props.size() ; j++ ) plyfile->elems[i].props[j].store = DONT_STORE_PROP;
		plyfile->elems[i].other_offset = NO_OTHER_PROPS; /* no "other" props by default */
	}

	/* set return values about the elements */
	elem_names.resize( plyfile->elems.size() );
	for( unsigned int i=0 ; i<elem_names.size() ; i++ ) elem_names[i] = plyfile->elems[i].name;
...
#if _WIN32 || _WIN64
	FILE *fp = NULL;
	if( fopen_s( &fp , name.c_str() , "rb" ) ) MK_ERROR_OUT( "Failed to open file for reading: " , name );
#else // !_WIN32 && !_WIN64
...
std::vector< PlyProperty * > PlyFile::get_element_description( const std::string &elem_name , size_t &nelems )
{
	...
	for( unsigned int i=0 ; i<elem->props.size() ; i++ ) prop_list[i] = new PlyProperty( elem->props[i].prop );
...
inline void setup_other_props( PlyElement *elem )
{
	...
	for( int type_size=8 ; type_size>0 ; type_size/=2 )
	{
		...
		for( unsigned int i=0 ; i<elem->props.size() ; i++ )
		{
...
bool PlyFile::set_other_properties( const std::string &elem_name , int offset , PlyOtherProp &other )
{
	...
	for( unsigned int i=0 ; i<elem->props.size() ; i++ ) if( !elem->props[i].store ) other.props.push_back( elem->props[i].prop );
...
PlyOtherElems *PlyFile::get_other_element( std::string &elem_name , size_t elem_count )
{
	...
	for( unsigned int i=0 ; i<other->other_data.size() ; i++ )
	{
...
void PlyFile::describe_other_elements( PlyOtherElems *other_elems )
{
	...
	for( unsigned int i=0 ; i<other_elems->other_list.size() ; i++ )
	{
		PlyElement elem;
...
void PlyFile::put_other_elements( void )
{
	...
	for( unsigned int i=0 ; i<other_elems->other_list.size() ; i++ )
	{
		...
		for( unsigned int j=0 ; j<other->other_data.size() ; j++ ) put_element( (void *)&other->other_data[j] );
	}
}
...
{
	for( unsigned int i=0 ; i<elems.size() ; i++ ) if( element==elems[i].name ) return &elems[i];
	return NULL;
}
...
{
	for( unsigned int i=0 ; i<props.size() ; i++ ) if( prop_name==props[i].prop.name ){ index = i ; return &props[i].prop; }
	index = -1;
	return NULL;
}
...
void PlyFile::_ascii_get_element( void *elem_ptr )
{
	...
	if( !words.size() ) MK_ERROR_OUT( "Unexpected end of file" );
...
inline void get_native_binary_type( void )
{
	...
	else MK_ERROR_OUT( "Couldn't determine machine endianness" );
}
...
inline void check_types()
{
	if( (ply_type_size[PLY_CHAR] != sizeof(char)) ||
	   ...
	   (ply_type_size[PLY_DOUBLE] != sizeof(double)))
		MK_ERROR_OUT( "Type sizes do not match built-in types" );
...
inline double get_item_value( const void *item , int type )
{
	switch( type )
	{
		...
		default: MK_ERROR_OUT( "Bad type: " , type );
	}
	return 0;
}
...
inline void write_binary_item( FILE *fp , int file_type , int int_val , unsigned int uint_val , long long longlong_val , unsigned long long ulonglong_val , double double_val , int type )
{
	...
	if( fwrite( value , ply_type_size[type] , 1 , fp )!=1 ) MK_ERROR_OUT( "Failed to write binary item" );
}
...
inline void write_ascii_item( FILE *fp , int int_val , unsigned int uint_val , long long longlong_val , unsigned long long ulonglong_val , double double_val , int type )
{
	switch (type)
	{
		...
		case PLY_INT:
		case PLY_INT_32:
			if( fprintf( fp , "%d " , int_val )<=0 ) MK_ERROR_OUT( "fprintf() failed -- aborting" );
			break;
		case PLY_LONGLONG:
		case PLY_INT_64:
			if( fprintf( fp , "%lld " , longlong_val )<=0 ) MK_ERROR_OUT( "fprintf() failed -- aborting" );
			break;
		case PLY_UCHAR:
		case PLY_UINT_8:
		case PLY_USHORT:
		...
		case PLY_UINT:
		case PLY_UINT_32:
			if( fprintf( fp , "%u " , uint_val )<=0 ) MK_ERROR_OUT( "fprintf() failed -- aborting" );
			break;
		case PLY_ULONGLONG:
		case PLY_UINT_64:
			if( fprintf( fp , "%llu " , ulonglong_val )<=0 ) MK_ERROR_OUT( "fprintf() failed -- aborting" );
			break;
			case PLY_FLOAT:
		case PLY_FLOAT_32:
		case PLY_DOUBLE:
		case PLY_FLOAT_64:
			if( fprintf( fp , "%g " , double_val )<=0 ) MK_ERROR_OUT( "fprintf() failed -- aborting" );
			break;
		default: MK_ERROR_OUT( "Bad type: " , type );
	}
}
...
inline void get_binary_item( FILE *fp , int file_type , int type , int &int_val , unsigned int &uint_val , long long &longlong_val , unsigned long long &ulonglong_val , double &double_val )
{
	...
	if( fread( ptr , ply_type_size[type] , 1 , fp )!=1 ) MK_ERROR_OUT( "fread() failed -- aborting." );
...
void PlyFile::add_element( const std::vector< std::string > &words )
{
	...
	/* set the new element */
	elem.name = words[1];
	elem.num = (size_t)std::stoull( words[2] );
	elem.props.resize(0);
